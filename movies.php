    ...
    <section>
        ...
        <div class="card-grid">
            <?php foreach ($movies as $movie): ?>
                <a class="channel-card" href="/movie.php?id=<?php echo $movie['id']; ?>">
                    <div class="channel-logo">
                        <?php if (!empty($movie['poster_url'])): ?>
                            <img src="<?php echo sanitize($movie['poster_url']); ?>" alt="<?php echo sanitize($movie['title']); ?>" style="max-width:90px; max-height:70px; border-radius:6px;">
                        <?php else: ?>
                            <i class="fas fa-film"></i>
                        <?php endif; ?>
                    </div>
                    <div class="channel-info">
                        <h3 class="channel-title"><?php echo sanitize($movie['title']); ?></h3>
                        <p class="channel-category" style="color:var(--muted);"><?php echo (int)$movie['year']; ?> • <?php echo sanitize($movie['genre']); ?></p>
                        <?php if ($movie['rating']): ?><span class="badge">Rating <?php echo $movie['rating']; ?></span><?php endif; ?>
                    </div>
                </a>
            <?php endforeach; ?>
        </div>
